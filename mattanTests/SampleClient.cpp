...
static const int RANDOM_REPEATS = 50;
  ...
  for (int i = 0; i < REPEATS; ++i)
    {
      ...
      JobState state;
      JobState last_state={UNDEFINED_STAGE,0};
      JobContext* job = static_cast<JobContext *>(startMapReduceJob(client, client.inputVec, client.outputVec, 6));
      getJobState(job, &state);
      ...
      while (state.stage != REDUCE_STAGE || state.percentage != 100.0)
        {
          if (last_state.stage != state.stage || last_state.percentage != state.percentage) {
              ...
              if (last_state.stage == state.stage && state.percentage < last_state.percentage) {
                  int a = job->counters[MAP_STAGE].second.load();
                  int b = job->counters[SHUFFLE_STAGE].second.load();
  ...
  for (int i = 0; i < DEADLOCK_REPEATS; ++i)
    {
      ...
      JobState state;
      JobState last_state={UNDEFINED_STAGE,0};
      JobHandle job = startMapReduceJob(client, client.inputVec, client.outputVec, 70);
      getJobState(job, &state);
      while (state.stage != REDUCE_STAGE || state.percentage != 100.0)
        {
          last_state = state;
